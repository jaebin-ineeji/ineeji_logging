...
class Logger:
    ...
    def error(self, message: Any, *args, exc_info=True, **kwargs):
        """에러 레벨 로그 메시지"""
        self.logger.error(message, *args, exc_info=exc_info, stacklevel=2, **kwargs)
    
    def critical(self, message: Any, *args, exc_info=True, **kwargs):
        """치명적 레벨 로그 메시지"""
        self.logger.critical(message, *args, exc_info=exc_info, stacklevel=2, **kwargs)
